...
def recursive_draw(tur, x, y, length, angle, count):
    ...
    if count <= 0:
        return
    if count == FRACTAL_DEPTH:
        draw_line(tur, (x, y), (x, y + length))
    left_x, left_y = draw_line_in_angle(tur, length, angle + 30)
    right_x, right_y = draw_line_in_angle(tur, length, angle - 30)
    tur.penup()
    tur.goto(left_x, left_y)
    tur.pendown()
    recursive_draw(tur, left_x, left_y, length * 0.5, angle + 30, count - 1)
    
    tur.penup()
    tur.goto(right_x, right_y)
    tur.pendown()
    recursive_draw(tur, right_x, right_y, length * 0.5, angle - 30, count - 1)
